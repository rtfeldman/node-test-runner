module.exports =
  function(elmPath, moduleName) {
    ...
    // Run the Elm app. Stringify these values to prevent code injection.
    "    var supervisor = new Supervisor(" + JSON.stringify(elmPath) + ", " + JSON.stringify(moduleName) + ");\n" +

    // Keep running until we explicitly exit.
    "    setInterval(function() {}, 30000);\n" +

    // Receive messages from ports and translate them into appropriate JS calls.
    "    supervisor.on('FINISH', function(data) {\n" +
    "        console.log(data.message);\n" +
    "        process.exit(data.exitCode);\n" +
    "    });\n" +

    "    supervisor.on('CHALK', function(data) {\n" +
