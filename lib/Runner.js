      ...
      switch (matchingSourceDirs.length) {
        case 0:
          return Promise.reject(
            Error(
              isPackageProject
                ? packageTestOutsideTestsDirError(filePath)
                : missingSourceDirectoryError(filePath)
            )
          );
...
function packageTestOutsideTestsDirError(filePath) {
  return `
This file:

${filePath}

…is not located in the tests/ folder! Package tests must be in the tests/ folder.
  `.trim();
}

function missingSourceDirectoryError(filePath) {
  return `
This file:
