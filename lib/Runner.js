      ...
      proc.on('close', function(code) {
        ...
        try {
          ...
          if (outline.type !== "ValidPkg") {
            reject('Invalid package - please run `elm make` instead of `elm test` and fix the errors you see!');
          } else {
      ...
      proc.on('close', function(code) {
        ...
        var filteredModules = _.flatMap(modules, function(mod) {
          ...
          if (eligible.length > 0) {
            return [{ moduleName: mod.module, path: path.resolve(path.join(testRootDir, mod.path)), tests: eligible }];
          } else {
            return [];
          }
        });
...
function filterExposing(pathAndModule /*: { moduleName: string, path: string, tests: Array<string> } */) /*: Promise<{ moduleName: string, path: string, tests: Array<string> }>*/{
  return new Promise(function(resolve, reject) {
    return finder
      ...
      .then(function(exposedValues) {
        ...
        if (newTests.length < pathAndModule.tests.length) {
          ...
          return resolve({ moduleName: pathAndModule.moduleName, path: pathAndModule.path, tests: newTests });
        }
      })
      .catch(reject);
  });
