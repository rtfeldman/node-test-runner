    ...
    function handleResponse(response, send) {
      switch (response.type) {
        ...
        case 'BEGIN':
          ...
          printResult(response.message);
          if (response.bufferedDebugLogs.length > 0) {
            process.stderr.write(response.bufferedDebugLogs);
            if (report === 'console') {
              console.error('\n');
            }
          }
        ...
        case 'RESULT': {
            ...
            case 'unit':
              if (response.expectationElmCode !== null) {
                cachedTests.unitTests.push({
                  ...
                  bufferedDebugLogs: response.bufferedDebugLogs,
                });
              }
              break;
          ...
          if (unitTests === undefined || fuzzTests === undefined) {
            // Not READY yet: We got a cached result.
            break;
          }

          // Only count non-cached towards being finished with running tests for real.
          switch (response.testType) {
            case 'unit':
              finishedUnitTests++;
              break;
