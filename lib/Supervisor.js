...
function run(
  ...
  return new Promise(function (resolve) {
    ...
    var workers = [];
    /** @type { { fuzzRuns: number, initialSeed: number, fingerprints: Record<string, { hash: string, outcomes: Array<string> }> } } */
    var toBePreviousRun = {
      fuzzRuns: fuzz,
      initialSeed: seed,
      fingerprints: {},
    ...
    for (var key in hashes) {
      toBePreviousRun.fingerprints[key] = {
        hash: hashes[key],
        outcomes: [],
      };
    }
    ...
    function handleResults(response) {
      ...
      for (const newStuff of response.newStuff) {
        toBePreviousRun.fingerprints[newStuff.jsDefinitionName].outcomes.push(
          newStuff.dictTupleElmCode
        );
      }
