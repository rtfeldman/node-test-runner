...
function findTests(
  ...
  verbose /*: boolean */
) /*: Promise<{exactDeps: Object, tests: Array<Object>}> */ {
    ...
    function finish() {
      ...
      proc.on('close', function(code) {
        ...
        try {
          ...
          return reject('Received invalid JSON from test interface search: ' + err);
        }

        const outline = modules.details.outline;
        const exactDeps = outline.hasOwnProperty("exactDeps") ? outline.exactDeps : null;

        var filteredModules = _.flatMap(modules.internals, function(mod) {
          var eligible = _.flatMap(_.toPairs(mod.values), function(
            pair
          ) {
            var name = pair[0];
            var annotation = pair[1].annotation;
            if (
              // TODO check if moduleName always exists
              annotation.moduleName &&
              // TODO check if moduleName.package always exists
          ...
          if (eligible.length > 0) {
            return [{ name: mod.module, tests: eligible }];
          } else {
            return [];
          }
        });
        ...
        return verifyModules(testFilePaths)
          .then(function() {
            return Promise.all(
              ...
              .then(function(tests) {
                console.log("returning:", {tests: tests, exactDeps: exactDeps});
                resolve({tests: tests, exactDeps: exactDeps});
              })
              .catch(reject);
          })
          .catch(reject);
