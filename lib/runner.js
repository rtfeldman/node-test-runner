...
function toPathsAndModules(testFilePaths) {
  ...
  return function(testModule) {
    var testModulePaths = moduleFromTestName(testModule.name);
    var results = [];
    ...
    paths.forEach(function(currentPath) {
      ...
      if (testModulePaths.every(function(testModulePath, index) {
        return testModulePath === currentPath.module[index];
      })) {
        results.push({
...
function findTests(elmPackageJsonPath, testFilePaths) {
  return new Promise(function(resolve, reject) {
    ...
    process.on('close', function(code) {
      ...
      return verifyModules(testFilePaths).then(function() {
        return Promise.all(
          _.map(_.flatMap(filteredModules, toPathsAndModules(testFilePaths)), filterExposing)
