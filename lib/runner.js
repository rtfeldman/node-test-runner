      ...
      switch (state) {
        ...
        case "INSIDE_EXPOSES":
          ...
          while (matches = data.match(/^(\s*..\s*|(?:\s*(?:[a-z][^\),\s]*))|(?:\s*(?:[A-Z][^\_,\s]*[^\(]*(?:\(?:[^\)]+\))?)[,]?\s*))([,\)])(.*)/)) {
            ...
            if (exposedValue.toLowerCase() === exposedValue) {
...
function filterTest(filePathArgs) {
  var modulePaths = filePathArgs.map(moduleFromFilePath);

  return function(test) {
    var testModulePaths = moduleFromTestName(test.name);

    // Must have at least one matching module path.
    return modulePaths.some(function(modulePath) {
      // A module path matches if it lines up completely with a known one.
      return testModulePaths.every(function(testModulePath, index) {
...
function findTests(elmPackageJsonPath, filePathArgs) {
  return new Promise(function(resolve, reject) {
    ...
    process.on('close', function(code) {
      ...
      return resolve(filePathArgs.length > 0 ? filteredModules.filter(filterTest(filePathArgs)) : filteredModules);
    });
  });
}
