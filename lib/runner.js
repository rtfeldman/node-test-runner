        ...
        fs.readdir(srcPath, function(err, filenames) {
          if (err) {
            ...
            Promise.all(filenames.map(function(filename) {
              return addSrcFilesHelp(filename, srcModules, visitedDirectories, srcPath, rootDir);
            })).then(resolve).catch(reject);
          }
        });
      } else {
        // It's allowed to have a source directory listed in elm-package.json
        // that never got created.
        resolve(null);
      }
    } else {
      // We're choosing to ignore this path.
      resolve(null);
    }
  });
}
...
function addSrcFilesHelp(filename, srcModules, visitedDirectories, srcPath, rootDir) {
  return new Promise(function(resolve, reject) {
    ...
    if (isNativeDir) {
      ...
      fs.lstat(filePath, function(err, stats) {
        if (err) {
          ...
          addSrcFilesTo(srcModules, visitedDirectories, filePath, rootDir).then(resolve).catch(reject);
          ...
          // This isn't a file we care about, so move on.
          resolve(null);
        }
      });
    }
