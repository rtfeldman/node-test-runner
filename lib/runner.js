...
function findTests(elmPackageJsonPath, testFilePaths, sourceDirs) {
  ...
  return new Promise(function(resolve, reject) {
    if (fs.existsSync(readElmiPath)) {
      ...
      return finish().then(resolve).catch(reject);
    } else {
      // it wasn't downloaded, possibly because we were installed with
      // --ignore-scripts - so download it!
      return installReadElmi().then(finish).catch(function(errorMessage) {
        console.error("Error installing elm-interface-to-json:", errorMessage);
        process.exit(1);
      });
    }
  });
