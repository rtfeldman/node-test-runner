...
class OnlineAvailableVersionLister {
  ...
  list(pkg /*: string */, indirectDeps /*: undefined | Map<string, string>*/) /*: Array<string> */ {
    const memoVersions = this.memoCache.get(pkg);
    if (memoVersions !== undefined) {
      return prioritizePinnedIndirectVersion(memoVersions, indirectDeps?.[pkg]);
    }
    ...
    return prioritizePinnedIndirectVersion(allVersions, indirectDeps?.[pkg]);
  }
}
...
class OfflineAvailableVersionLister {
  ...
  list(pkg /*: string */, indirectDeps /*: undefined | Map<string, string>*/) /*: Array<string> */ {
    ...
    return prioritizePinnedIndirectVersion(offlineVersions, indirectDeps?.[pkg]);
  }
}
  ...
  solveOffline(
    ...
    try {
      return wasm.solve_deps(
        ...
        extra,
        fetchElmJsonOffline,
        (pkg) => lister.list(pkg, indirectDeps)
      );
    } catch (errorMessage) {
...
function prioritizePinnedIndirectVersion(
  ...
  pinnedVersion /*: undefined | string */) /*: Array<string> */ {
  if (!pinnedVersion || !versions.includes(pinnedVersion)) {
    return versions;
  }

  // the pinned version and any newer version, in ascending order
  const desirableVersions = versions.filter(v => v >= pinnedVersion).reverse();
