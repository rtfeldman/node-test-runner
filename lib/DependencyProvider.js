...
// Constructor of an offline solver.
function newOffline() /*: {
  solve: (string, boolean, { [string]: string }) => string,
} */ {
  function solve(elmJson, useTest, extra) {
...
// Constructor of an online solver.
function newOnline() /*: {
  solve: (string, boolean, { [string]: string }) => string,
} */ {
  function solve(elmJson, useTest, extra) {
...
function listAvailableVersionsOnline(pkg /*: string */) /*: Array<string> */ {
  ...
  const allVersions = [...allVersionsSet].sort(compareStringVersions).reverse();
...
function listAvailableVersionsOffline(pkg /*: string */) /*: Array<string> */ {
  ...
  offlineVersions.sort(compareStringVersions).reverse();
...
// Return +1 if v1 > v2
//
// Versions are strings expecting to be simple SemVer versions
// of the shape "Major.Minor.Patch".
function compareStringVersions(
  v1 /*: string */,
  v2 /*: string */
) /*: number */ {
  const [major1, minor1, patch1] = v1.split('.');
  const [major2, minor2, patch2] = v2.split('.');
  let left = parseInt(major1);
  let right = parseInt(major2);
  if (left < right) {
    return -1;
  } else if (left > right) {
    return 1;
  } else {
    left = parseInt(minor1);
    right = parseInt(minor2);
    if (left < right) {
      return -1;
    } else if (left > right) {
      return 1;
    } else {
      left = parseInt(patch1);
      right = parseInt(patch2);
      if (left < right) {
        return -1;
      } else if (left > right) {
        return 1;
