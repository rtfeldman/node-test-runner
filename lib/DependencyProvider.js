...
class AvailableVersionLister {
  ...
  /**
   * @param { string } pkg
   * @param { OrderConstraints } orderConstraints
   * @returns { Array<string> }
   */
  list(pkg, orderConstraints) {
    const memoVersions = this.memoCache.get(pkg);
    if (memoVersions !== undefined) {
      return applyOrderConstraints(memoVersions, orderConstraints);
    }
    ...
    return applyOrderConstraints(allVersions, orderConstraints);
  }
}
...
/**
 * When doing `import('./DependencyProvider').DependencyProvider`,
 * TypeScript says `DependencyProvider` is not exported for some reason.
 * But `import('./DependencyProvider').DependencyProviderType` works.
 * @typedef { DependencyProvider } DependencyProviderType
 */
...
class DependencyProvider {
   ...
   * @param { PackageStrategy } packageStrategy
   * @returns { string }
   */
  solve(offline, elmJson, useTest, extra, packageStrategy) {
    const lister = new AvailableVersionLister(this.cache);
    ...
    try {
      return wasm.solve_deps(
        ...
        offline
          ? fetchElmJsonOffline
          : fetchElmJsonOnline.bind(null, this.syncHttpGet),
        /** @type { (pkg: string) => Array<string> } */
        (pkg) =>
          lister.list(pkg, toOrderConstraints(elmJson, packageStrategy, pkg))
      );
    } catch (errorMessage) {
      throw new Error(`Failed to solve dependencies:\n${errorMessage}`);
    }
  }
}
...
/**
 * @typedef {
    | { for: 'application', pinnedVersion: string | undefined }
 ...
 * @param { PackageStrategy } packageStrategy
 * @param { string } pkg
 * @returns { OrderConstraints }
 */
function toOrderConstraints(elmJson, packageStrategy, pkg) {
  switch (elmJson.type) {
    case 'application':
      return {
        for: 'application',
        pinnedVersion: elmJson.dependencies.indirect[pkg],
      };
 ...
 * @returns { Array<string> }
 */
function applyOrderConstraints(versions, orderConstraints) {
  switch (orderConstraints.for) {
    case 'application':
      return prioritizePinnedIndirectVersion(
        versions,
        orderConstraints.pinnedVersion
      );

    case 'package':
      switch (orderConstraints.strategy) {
        case 'newest':
          return versions;
...
module.exports = {
  ...
  prioritizePinnedIndirectVersion: prioritizePinnedIndirectVersion,
};
