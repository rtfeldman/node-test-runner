...
function runElmTest() {
  ...
  if (args._[0] == "init") {
    ...
    if (packagesToInstall.length > 0) {
      var cmd = [pathToElm, "install"].concat(packagesToInstall).join(" ");
...
function generatePackageJson(filePathArgs) {
  ...
  function addDependencies(deps) {
    Object.keys(deps).forEach(function(name) {
      ...
      testElmJson.dependencies[name] = version;
    });
  }

  function addTransitiveDependencies(deps) {
    Object.keys(deps).forEach(function(name) {
      if (testElmJson.dependencies.hasOwnProperty(name)) {
        // already a normal dep
      } else {
        var version = deps[name];
        testElmJson["do-not-edit-this-by-hand"]["transitive-dependencies"][name] = version;
      }
    });
  }
  ...
  addDependencies(nodeTestRunnerElmJson.dependencies);
  addDependencies(projectElmJson.dependencies);
  addDependencies(projectElmJson["test-dependencies"]);
  addTransitiveDependencies(nodeTestRunnerElmJson["do-not-edit-this-by-hand"]["transitive-dependencies"]);
  if (isPackageProject) {
    // package projects don't explicitly list their transitive dependencies,
    // to we have to figure out what they are
    fs.writeFileSync(
      path.join(generatedCodeDir, "elm.json"),
      JSON.stringify(testElmJson, null, 4)
    );
    var result = child_process.spawnSync(pathToElm, ["make", "--report=json"], {
      silent: true,
      cwd: generatedCodeDir
    });
    var output = result.stderr.toString(); // TODO: hopefully the next Elm 0.19 beta will print the JSON to stdout instead
    ...
    if (report.type === "error" && report.title === "NO INPUT") {
      // all transtive dependencies are listed already
      ...
      addTransitiveDependencies(missingDeps);
    } else {
    ...
    addTransitiveDependencies(projectElmJson["do-not-edit-this-by-hand"]["transitive-dependencies"]);
  }
...
function generateAndRunTests(tests, filePathArgs, generatedSrc, getGlobs) {
  ...
  if (args.watch) {
    ...
    if (elmJson["type"] === "package") {
      watchedSourcePaths = [ resolveWatchPath(projectRootDir)("./src") ];
    } else {
      watchedSourcePaths = elmJson["source-directories"].map(resolveWatchPath(projectRootDir));
    }
