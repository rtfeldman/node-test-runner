...
function generatePackageJson(filePathArgs) {
  ...
  function ensureTransitivePackage(name, version) {
    if (elmPackageContents.dependencies.hasOwnProperty(name)) {
      // already a normal dep
    } else if (elmPackageContents["do-not-edit-this-by-hand"]["transitive-dependencies"].hasOwnProperty(name)) {
      // already a transitive dep
    } else {
      elmPackageContents["do-not-edit-this-by-hand"]["transitive-dependencies"][name] = version;
    }
  }

  Object.keys(elmPackageContents["test-dependencies"]).forEach(function(name) {
    var version = elmPackageContents["test-dependencies"][name];
  ...
  ensureTransitivePackage("eeue56/elm-lazy", "1.0.1");
  ensureTransitivePackage("eeue56/elm-lazy-list", "2.0.0");
  ensureTransitivePackage("eeue56/elm-shrink", "2.0.0");
  ensureTransitivePackage("elm/html", "1.0.0"); // TODO: elm-lazy shouldn't actually depend on this
  ensureTransitivePackage("elm/virtual-dom", "1.0.0"); // TODO: elm-lazy shouldn't actually depend on this

  elmPackageContents["test-dependencies"] = {};
  ...
  elmPackageContents["source-directories"] =
    elmPackageContents["source-directories"].filter(function(value, index, self) {
