...
function makeAndTestHelper(
  ...
  try {
    const projectElmJson = JSON.parse(fs.readFileSync(elmJsonPath, 'utf8'));
    return {
      testFilePaths,
      generatedCodeDir,
      projectElmJson,
    ...
    throw new Error(`Error reading elm.json: ${error.message}`);
  }
}
...
async function make(
  ...
  const { testFilePaths, generatedCodeDir, projectElmJson } = makeAndTestHelper(
    projectRootDir,
    testFileGlobs
  );

  Generate.generateElmJson(projectRootDir, generatedCodeDir, projectElmJson);
...
function test(
  ...
  async function run() {
    try {
      const {
        ...
        generatedCodeDir,
        projectElmJson,
      } = makeAndTestHelper(projectRootDir, testFileGlobs);

      if (testFilePaths.length === 0) {
        throw new Error(noFilesFoundError(projectRootDir, testFileGlobs));
      }

      if (watcher !== undefined) {
        const nextGlobsToWatch = getGlobsToWatch(projectElmJson);
      ...
      const dest = path.join(generatedCodeDir, 'elmTestOutput.js');
  ...
  if (watch) {
    ...
    const initialGlobsToWatch = [path.join(projectRootDir, 'elm.json')];
...
function main() {
  ...
  const getClosestElmJsonPath = (subcommand /*: string */) /*: string */ => {
    const elmJsonPath = findClosest('elm.json', process.cwd());
...
(If elm is not on your path or is called something different the --compiler flag might help.)`
  ...
  program
    ...
    .action((cmd) => {
      ...
      const elmJsonPath = getClosestElmJsonPath('init');
      const projectRootDir = path.dirname(elmJsonPath);
      const testsDir = path.join(projectRootDir, 'tests');
      Install.install(projectRootDir, pathToElmBinary, 'elm-explorations/test');
      fs.mkdirSync(testsDir, { recursive: true });
      fs.copyFileSync(
        path.join(__dirname, '..', 'templates', 'tests', 'Example.elm'),
        path.join(testsDir, 'Example.elm')
      );
      console.log(
  ...
  program
    ...
    .action((packageName, cmd) => {
      ...
      const options = program.opts();
      const pathToElmBinary = getPathToElmBinary(options.compiler);
      const elmJsonPath = getClosestElmJsonPath('install');
      const projectRootDir = path.dirname(elmJsonPath);
      Install.install(projectRootDir, pathToElmBinary, packageName);
      process.exit(0);
    });
  ...
  program
    ...
    .action((testFileGlobs) => {
      const options = program.opts();
      const pathToElmBinary = getPathToElmBinary(options.compiler);
      const elmJsonPath = getClosestElmJsonPath('make');
      const projectRootDir = path.dirname(elmJsonPath);
      make(projectRootDir, pathToElmBinary, testFileGlobs, options.report).then(
        () => process.exit(0),
        () => process.exit(1)
      );
    });
