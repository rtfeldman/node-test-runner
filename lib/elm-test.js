...
// Never print verbose output when printing to a machine; otherwise we will
// end up with effectively malformed XML (or JSON) going to stdout!
var verbose = args.verbose && !isMachineReadableReporter(args.report);
...
function runElmTest() {
  ...
  if (
    !fs.existsSync(newElmStuffPath) &&
    !fs.lstatSync(newElmStuffPath).isSymbolicLink()
  ) {
    fs.symlinkSync(
