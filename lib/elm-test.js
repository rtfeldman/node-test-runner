...
const { DependencyProvider } = require('./DependencyProvider');
 ...
 * @returns { string | void }
 */
function findClosestElmJson(dir) {
 ...
 * @returns { string }
 */
function getProjectRootDir(subcommand) {
...
/**
 * @param { string } subcommand
 * @returns { import('./Project').Project }
 */
function getProject(subcommand) {
  try {
...
/**
 * @param { string | void } compiler
 * @returns { string }
 */
function getPathToElmBinary(compiler) {
  const name = compiler === undefined ? 'elm' : compiler;
  try {
    // If you pass a relative path, it should be relative to the current CWD.
    // We run the Elm compiler with a generated directory as CWD, so we have
    // to resolve the path ourselves first. We can’t just use `path.resolve()`
    // since we want a plain word like `lamdera` to both refer to a file with
    // that name somewhere in `$PATH` and to a file in the current directory.
...
function main() {
  ...
  program
    ...
    .option(
      '--unbuffered-logs',
      'Print debug logs immediately instead of at the end of each test',
      false
    )
    ...
    .addOption(
      new Option(
        ...
        'Choose newest or oldest compatible dependencies for a package project (ignored for applications)'
      )
        .default('newest')
    ...
    .option(
      '--offline',
      'Do not make any HTTP requests when solving dependencies'
    )
    ...
    .action((testFileGlobs) => {
      ...
      const make = async () => {
        Generate.generateElmJson(
          ...
          options['offline'],
          () => {
            // Don’t log that we are solving dependencies in `make` mode.
          }
        );
