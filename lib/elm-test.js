...
function compileAllTests(testFilePaths) {
  return new Promise(function(resolve, reject) {
    // Make sure rtfeldman/console-print is installed
    spawn.sync("elm-package", ["install", "--yes"], { cwd: generatedCodeDir });
...
function generatePackageJson(filePathArgs) {
  ...
  if (
    !elmPackageContents.dependencies.hasOwnProperty("rtfeldman/console-print")
  ) {
    // Console.Text needs this.
    Object.assign(elmPackageContents.dependencies, {
      "rtfeldman/console-print": "1.0.0 <= v < 2.0.0"
    });
  }
