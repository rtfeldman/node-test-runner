...
function runTests(repository, testFile) {
...
function prepareCompiledJsFile(repository, initialSeed, report, dest) {
  ...
  return Promise.all(promises).then(function(values) {
    ...
    var nativePackageName = repositoryToNativePackageName(repository);

    overrides = ["_", nativePackageName, "$", overrides].join("");
...
function runElmTest() {
  ...
  var repository = returnValues[0];
  var newElmPackageDir = returnValues[1];
  var generatedSrc = returnValues[2];
  var sourceDirs = returnValues[3];
...
function generatePackageJson(filePathArgs) {
  ...
  var repository = elmPackageContents.repository;
  ...
  return [repository, newElmPackageDir, generatedSrc, sourceDirs];
}
