...
function test(
  ...
  async function run() {
    try {
      ...
      const elmJson = ElmJson.read(project.rootDir);
  ...
  if (watch) {
    ...
    const initialGlobsToWatch = [ElmJson.getPath(project.rootDir)];
...
function main() {
  ...
  const getProject = (
    ...
    try {
      ...
      return Project.init(path.dirname(elmJsonPath), packageInfo.version);
    } catch (error) {
...
(If elm is not on your path or is called something different the --compiler flag might help.)`
  ...
  program
    ...
    .action((testFileGlobs) => {
      ...
      make(project, pathToElmBinary, testFileGlobs, options.report).then(
        () => process.exit(0),
        () => process.exit(1)
      );
    });
  ...
  program
    ...
    .action((testFileGlobs) => {
      ...
      test(project, pathToElmBinary, testFileGlobs, processes, options);
    });
