...
if (args._[0] === 'make') {
  ...
  // TODO this probably now needs to use generated dir
  Compile.compileAll(
    testFilePaths,
    projectRootDir,
    args.verbose,
  ...
  if (isPackageProject) {
    ...
    Compile.compileSources([], projectRootDir, args.verbose, pathToElmBinary, args.report).then(() => {
      Runner.getIndirectDeps(projectRootDir).then((packageIndirectDeps) => {
        return generateAndRun(projectRootDir, testFilePaths, packageIndirectDeps, hasBeenGivenCustomGlobs);
      });
    });
  } else {
