...
function test(
  ...
  async function run(events /*: typeof Queue */) {
    try {
      ...
      const onlyChanged =
        events.length > 0 && events.every(({ event }) => event === 'changed');
      if (!onlyChanged) {
  ...
  if (watch) {
    ...
    const onRunFinish = () => {
      ...
      if (currentRun.queue.length > 0) {
        clearConsole(report);
        infoLog(report, watcherEventMessage(currentRun.queue));
        currentRun = {
          promise: run(currentRun.queue).then(onRunFinish),
          queue: [],
        };
      } else {
    ...
    const rerun = (event) => (filePath) => {
      const queue = [{ event, filePath }];
      if (currentRun === undefined) {
        clearConsole(report);
        infoLog(report, watcherEventMessage(queue));
        currentRun = { promise: run(queue).then(onRunFinish), queue: [] };
      } else {
        currentRun.queue.push(...queue);
      }
    };

    // There are events for adding and removing directories as well, but that
    // never affects tests (adding/removing a directory with files in it
    // produces events for both the directory and all the files). Besides, our
    // watched globs only match elm.json and .elm files so the directory events
    // don’t even trigger.
    ...
    currentRun = { promise: run([]).then(onRunFinish), queue: [] };
  } else {
    run([]);
  }
}
