...
function main() {
  ...
  program
    ...
    .description('Use the `master` version of the elm-explorations/test library')
    .action(() => { 
      const project = getProject('install-unstable-test-master');
      Install.installUnstableTestMaster(project)
        .then(
          () => process.exit(0),
          () => process.exit(1)
        )
  ...
  program
    ...
    .description('Stop using the `master` version of the elm-explorations/test library')
    .action(() => { 
