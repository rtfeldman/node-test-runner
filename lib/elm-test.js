...
function newMainFileNeeded(hash) {
  var cachedHashFile = path.resolve(
    testRootDir,
    generatedCodeDir,
    "MAIN_CONTENTS_HASH"
  );
  ...
  if (
    !fs.existsSync(cachedHashFile) ||
    fs.readFileSync(cachedHashFile, "utf8") !== hash
  ) {
    fs.writeFileSync(cachedHashFile, hash, "utf8");
    return true;
  } else {
    return false;
  }
...
function generateAndRunTests(tests, filePathArgs, generatedSrc, getGlobs) {
  ...
  var hash = murmur.murmur3(testFileBody);
  var moduleName = "Main";
  ...
  var testFile = path.join(mainPath, moduleName + ".elm");

  if (newMainFileNeeded(hash)) {
