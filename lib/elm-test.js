...
function parseArgv(argv, resolveCompiler) {
  const options = {
    help: false,
    version: false,
    watch: false,
  ...
  const paddedArgv = argv.concat('nothing');
    ...
    switch (arg) {
      ...
      case 'compiler':
        if (index === lastIndex) {
          return {
            tag: 'Error',
            message: 'You must pass a path after --compiler',
          };
        }
        options.compiler = argv[index + 1];
        index++;
        break;

      case 'seed': {
        const result = parsePositiveInteger(paddedArgv[index + 1]);
        index++;
        switch (result.tag) {
          case 'Ok':
          ...
          case 'Error':
            return {
              tag: 'Error',
              message: `You must pass a number after --seed: ${result.message}`,
            };
        }
        break;
      }
      ...
      case 'fuzz': {
        ...
        switch (result.tag) {
          ...
          case 'Error':
            return {
              tag: 'Error',
              message: `You must pass a number after --fuzz: ${result.message}`,
            };
        }
        break;
      }
      ...
      case 'report': {
        const result = parseReport(paddedArgv[index + 1]);
        index++;
        switch (result.tag) {
          case 'Ok':
          ...
          case 'Error':
            return {
              tag: 'Error',
              message: `You must pass a reporter --report: ${result.message}`,
            };
        }
        break;
      }
...
function parsePositiveInteger(string) {
  ...
  return /^\d+$/.test(string) && Number.isFinite(number)
    ? { tag: 'Ok', value: number }
    : { tag: 'Error', message: `Expected digits only, but got: ${string}` };
}

const validReports = ['console', 'json', 'junit'];

function parseReport(string) {
...
function parseCommand(args, raw) {
  const first = args[0];
...
function getPathToElmBinary(compiler) {
  ...
  try {
    return { tag: 'Ok', value: which.sync(name) };
  } catch (_error) {
...
(If elm is not on your path or is called something different the --compiler flag might help.)`,
      ...
      : {
          tag: 'Error',
          message: 'The elm executable passed to --compiler must exist.',
        };
  }
}
...
function resolveGlobs(fileGlobs) {
  ...
  if (fileGlobs.length > 0) {
    ...
    // TODO: There’s another process.cwd() here?
...
function main() {
  ...
  switch (parsed.command.tag) {
    ...
    case 'make': {
      const result = helper(parsed.command.testFileGlobs);
      switch (result.tag) {
        case 'Ok':
    ...
    case 'test': {
      ...
      const result = helper(testFileGlobs);
      switch (result.tag) {
        case 'Ok':
