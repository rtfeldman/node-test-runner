...
function test(
  ...
  async function run(event /*: 'init' | 'added' | 'changed' | 'removed' */) {
    try {
      ...
      // re-calculate what files match the globs. In other words, if only a file
      // has changed there’s nothing to do.
  ...
  if (watch) {
    ...
    const rerun = (event) => (filePath) => {
      clearConsole(report);
      infoLog(report, `${filePath} ${event}. Rebuilding!`);

      // TODO if a previous run is in progress, wait until it's done.
      currentRun = currentRun.then(() => run(event));
    };
