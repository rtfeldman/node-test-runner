...
if (args.compiler !== undefined) {
  pathToMake = path.resolve(args.compiler);
...
function ensurePackagesInstalled(dir) {
  return new Promise(function(resolve, reject) {
    dns.lookup("elm-lang.org", function(err) {
      if (err && (err.code == "ENOTFOUND" || error.code == "EAI_AGAIN")) {
        // Either we're offline or elm-lang.org is blocked or something.
        // Regardless, don't bother trying to install packages.
        resolve();
      } else {
...
function generateAndRunTests(tests, filePathArgs, generatedSrc, getGlobs) {
  ...
  if (args.watch) {
    ...
    var watchedTestPaths = fs
      .readJsonSync(elmPackagePath, "utf8")
