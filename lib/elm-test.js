...
function runElmTest() {
  ...
  console.error(testRootDir, originalDir);
  ...
  return ensurePackagesInstalled(testRootDir)
    .then(function() {
      ...
      return ensurePackagesInstalled(newElmPackageDir);
    })
    .then(function() {
...
function generatePackageJson(filePathArgs) {
  ...
  ensurePackage("elm-lang/random", "1.0.0");
  ensurePackage("elm-lang/time", "1.0.0");
  ensurePackage("elm-lang/json", "1.0.0");
  ...
  // must use the appropriate package name from the elm.json we're copying.
...
function ensurePackagesInstalled(dir) {
  // This is no longer necessary in Elm 0.19
...
function generateAndRunTests(tests, filePathArgs, generatedSrc, getGlobs) {
  ...
  if (args.watch) {
    ...
    var watchedSourcePaths = fs
      .readJsonSync(path.join(originalDir, "elm.json"), "utf8")
      ["source-directories"].map(resolveWatchPath(originalDir));
