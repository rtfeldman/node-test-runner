...
function generatePackageJson(filePathArgs) {
  ...
  // TODO remove the next three conditionals once random-pcg has become core's new Random!
  if (!elmPackageContents.dependencies) {
    elmPackageContents.dependencies = {};
  }

  if (
    !elmPackageContents.dependencies.hasOwnProperty(
      "Skinney/elm-array-exploration"
    )
  ) {
    // Highlightable needs jinjor/elm-diff to render diffs; we have inlined jinjor/elm-diff
