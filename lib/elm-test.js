...
Options:
    --compiler /path/to/compiler
        Use a custom path to an Elm executable (defaults to \`elm\`)
    --seed INT
        Run with initial fuzzer seed
    --fuzz INT
        Run with each fuzz test performing this many iterations
    --report json
    --report junit
    --report console (default)
...
function resolveFilePath(filename) {
  const candidates = !fs.existsSync(filename)
    ? []
  ...
  return candidates.filter((candidate) =>
    candidate.split(path.sep).includes('elm-stuff')
  );
}
...
function main() {
  ...
  const parseResult = parseArgv(process.argv.slice(2));
