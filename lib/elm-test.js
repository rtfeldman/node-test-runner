 ...
 * we sometimes wrote a large amount of output (at the end of
 ...
 * test that had buffered. If they are never printed, the consumer
...
function main() {
  ...
  program
    ...
    .action((testFileGlobs) => {
      ...
      make().then(
        () => setExitCode(0),
        // `Compile.compileSources` rejects with an error like this on compile errors:
        // `elm make` failed with exit code 1.
        // We have never printed that part in `make` mode, so we ignore the error parameter here.
        () => setExitCode(1)
      );
    });
