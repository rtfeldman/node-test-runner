...
function runElmTest() {
  ...
  if (filePathArgs.length > 0) {
    getGlobs = function() { return _.flatMap(filePathArgs, globify); };
  } else {
    var root = path.join(path.resolve(Runner.findNearestElmPackageDir([process.cwd()])));
  ...
  if (testFilePaths.length === 0) {
    var errorMessage =
      filePathArgs.length > 0
        ? "No tests found for the file pattern \"" + filePathArgs.toString() + "\"\n\nMaybe try running elm-test with no arguments?"
  ...
  compileAllTests(testFilePaths).then(
    function() {
      return Runner.findTests(elmRootDir, testFilePaths, sourceDirs).then(function(runnableTests) {
        ...
        generateAndRunTests(runnableTests, filePathArgs, generatedSrc, getGlobs);
      }).catch(function(err) {
        console.error(err);
        process.exit(1);
      });
...
function generatePackageJson(filePathArgs) {
  ...
  if (!elmPackageContents.dependencies.hasOwnProperty("mgold/elm-random-pcg")) {
    ...
    Object.assign(elmPackageContents.dependencies, {"mgold/elm-random-pcg": "4.0.2 <= v < 6.0.0"});
  }
...
function generateAndRunTests(tests, filePathArgs, generatedSrc, getGlobs) {
  ...
  if (testList.length === 0) {
    var errorMessage =
      filePathArgs.length > 0
        ? "I couldn't find any exposed values of type Test in files matching \"" + filePathArgs.toString() + "\"\n\nMaybe try running elm-test with no arguments?"
  ...
  var opts =
    {
      ...
      reporter : typeof args.report === "undefined" ? "Nothing" : "Just " + sanitizedToString(args.report),
  ...
  if (args.watch) {
    ...
    var watchedSourcePaths = fs.readJsonSync(path.join(originalDir, "elm-package.json"), 'utf8')['source-directories'].map(
      resolveWatchPath(originalDir)
    );
    var watchedTestPaths = fs.readJsonSync(elmPackagePath, 'utf8')['source-directories'].map(
      resolveWatchPath(path.dirname(elmPackagePath))
    );
