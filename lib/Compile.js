...
function spawnCompiler(report /*:string*/) {
  return (pathToElm /*:string*/, processArgs/*:Array<string>*/, processOpts/*:Object*/) => {
    const finalOpts = Object.assign({env: {ELM_HOME: process.env.ELM_HOME}},
      processOpts,
      {
        stdio: [
          process.stdin,
...
function processOptsForReporter(reporter) {
  if (isMachineReadableReporter(reporter)) {
    return { env: {ELM_HOME: process.env.ELM_HOME}, stdio: ["ignore", "ignore", process.stderr] };
  } else {
    return { env: {ELM_HOME: process.env.ELM_HOME} };
  }
}
