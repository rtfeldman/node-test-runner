    ...
    worker.on("message", function(data) {
      ...
      switch (response.type) {
        case "FINISHED":
          ...
          } else if (finishedWorkers.size === 1) {
            // This was the first test to finish; tell the others to switch
            // into load balancing mode.

            workers.forEach(function(worker, workerIndex) {
              if (workerIndex !== index) {
                workers[index].send({ type: "LOAD_BALANCE" });
              }
            });
          }
          break;
