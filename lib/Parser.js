...
function backslashError(actual) {
  return new SyntaxError(
    `Expected one of \`${Array.from(backslashableChars).join(
      ' '
...
function tokenize(
  ...
  switch (tokenizerState.tag) {
    case 'Initial':
      switch (char) {
        ...
        case '=':
          return [tokenizerState, { tag: 'FlushToken', token: { tag: '=' } }];
        case "'":
          return [{ tag: 'CharStart' }, { tag: 'NoFlush' }];
        case '"':
          return [{ tag: 'StringStart' }, { tag: 'NoFlush' }];
        default:
          return [
            {
              tag: 'Initial',
    ...
    case 'MaybeMultilineComment{':
      switch (char) {
        case '-':
          return [{ tag: 'MultilineComment', level: 1 }, { tag: 'Flush' }];
        default:
          return [
    ...
    case 'CharEnd':
      switch (char) {
        case "'":
          return [{ tag: 'Initial', otherTokenChars: '' }, { tag: 'NoFlush' }];
        default:
          return expected('a single-quote', char);
      }
...
function parseModuleDeclaration(
  ...
  switch (lastToken) {
    case 'Nothing':
      ...
      return expected('NewChunk', token);
    ...
    case 'NewChunk':
      ...
      return expected('port/effect/module', token);
    ...
    case 'module':
      switch (token.tag) {
        case 'UpperName': // Single name.
        case 'Other': // With dots.
          return 'ModuleName';
      }
    ...
    case 'ModuleName':
      ...
      return expected('exposing', token);
