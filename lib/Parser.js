...
const maybeTestDefinitionStart = new RegExp(
...
function extractExposedMaybeTests(filePath) {
  return new Promise((resolve) => {
    ...
    if (exposing !== '..') {
      resolve(
        exposing.split(/[ ,]+/).filter((part) => nonCapitalizedName.test(part))
      );
      return;
    }

    resolve(content.match(maybeTestDefinitionStart) || []);
  });
}

module.exports = {
  extractExposedMaybeTests,
};
