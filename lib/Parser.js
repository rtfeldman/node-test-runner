 ...
 * @param { Generator<Buffer, void> } readChunked
 * @returns { Array<string> }
 */
function extractExposedPossiblyTests(filePath, readChunked) {
  /** @type { Array<string> } */
  const exposedPossiblyTests = [];
  /** @type { TokenizerState } */
  let tokenizerState = { tag: 'MaybeNewChunk' };
  /** @type { ParserState } */
  let parserState = {
  ...
  /**
   * @param { Token } [token]
   * @returns { OnParserTokenResult | undefined }
   */
  ...
  /**
   * @param { OnParserTokenResult } result
   * @returns { true }
   */
  ...
  function onParserToken(token) {
    ...
    switch (parserState.tag) {
      ...
      case 'Rest': {
        ...
        if (typeof result === 'string') {
          parserState.lastToken = result;
          if (result === 'PotentialTestDeclaration=') {
            exposedPossiblyTests.push(lastLowerName);
          }
          return undefined;
        }
        switch (result.tag) {
          case 'ParseError':
            return result;
          default:
            // Type assertion needed here due to there only being one error variant.
            unreachable(/** @type { never } */ (result), 'tag');
        }
      }
  ...
  try {
    for (const chunk of readChunked) {
      if (onData(chunk)) {
        break;
      }
    }
 ...
 * @returns { never }
 */
function unreachable(value, property) {
  throw new Error(
    `Unreachable: ${property === undefined ? value : value[property]}`
  );
}
 ...
 * @typedef {
    | { tag: 'Flush' }
    | { tag: 'FlushToken', token: Token }
  } Cmd
 */

/**
 * @param { string } char
 * @param { TokenizerState } tokenizerState
 * @returns { [ TokenizerState, Array<Cmd> ] | ParseError }
 */
 ...
 * @typedef {
    | {
        tag: 'ModuleDeclaration',
        lastToken: ModuleDeclarationLastToken,
      }
    | {
        tag: 'Rest',
        lastToken: RestLastToken,
      }
  } ParserState
 *
 * @typedef {
    ...
    | 'UpperName)'
    | ','
  } ModuleDeclarationLastToken
 */

/**
 * @param { Token } token
 * @param { ModuleDeclarationLastToken } lastToken
 * @returns {
    | ModuleDeclarationLastToken
    | { tag: 'NextParserState' }
    | OnParserTokenResult
 ...
 */
function parseModuleDeclaration(token, lastToken) {
  switch (lastToken) {
    case 'Nothing':
...
/**
 * @param { Token } token
 * @param { RestLastToken } lastToken
 * @returns { RestLastToken | ParseError }
 */
function parseRest(token, lastToken) {
  switch (lastToken) {
    case 'Initial':
      if (token.tag === 'NewChunk') {
        return 'NewChunk';
