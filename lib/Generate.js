...
function generatePreviousRunModule(previousRunModule, previousRun) {
  ...
  const toTuple = ({ labels, expectation, debugLogs }) => {
    const separator = makeDebugLogSeparator(debugLogs);
    return `
( ${indentAllButFirstLine('  ', makeList(labels.map(makeElmString)))}
, ( ${expectation}
  , Test.DebugLogs.fromString ${makeElmString(separator)}
      ${makeElmString(debugLogs.join(separator))}
  )
)
    `.trim();
  };
...
/**
 * @param { Array<string> } debugLogs
 * @returns { string }
 */
function makeDebugLogSeparator(debugLogs) {
  let separator = '@@@';
  while (debugLogs.some((log) => log.includes(separator))) {
    separator += Math.random().toString(16).slice(2);
  }
  return separator;
}
