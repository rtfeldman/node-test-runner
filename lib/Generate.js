...
function generateElmJson(
  ...
  // For applications, tests can exist in any source dir.
  // For packages, tests can only practically exists in `tests/` because if you
  // put tests in `src/` the package would have to unnecessarily depend on
  // elm-explorations/test to compile.
  return [generatedSrc, isPackageProject ? [testRootDir] : sourceDirs];
}
