...
function askElmForMissingTransitiveDependencies(pathToElmBinary, pathToElmProject) {
  var result = spawn.sync(pathToElmBinary, ["make", "--report=json"], {
    silent: true,
    env: {ELM_HOME: process.env.ELM_HOME},
    cwd: pathToElmProject
  });

  if (result.stderr == null) {
    console.error("No output received from elm make when searching for indirect dependencies.");
    process.exit(1);
  }
...
function generateMainModule(fuzz/*:number*/, seed/*:number*/, report/*:string*/, testFilePaths/*:Array<string>*/, tests/*:Array<Object>*/, generatedSrc/*:string*/, processes/*:number*/) {
