...
function prepareCompiledJsFile(testModules, pipeFilename, dest) {
  ...
  console.time('Hash.calculateHashes');
  const hashes = Hash.calculateHashes(names, content);
  console.timeEnd('Hash.calculateHashes');
...
`.trim()
      ...
      .replace(
        ...
        `
// TODO: isFuzzTest is never true
var _elmTestIsFuzzTest = false;
var $elm_explorations$test$Test$Fuzz$fuzzLoop_elmTestOriginal = typeof $elm_explorations$test$Test$Fuzz$fuzzLoop === "function" ? $elm_explorations$test$Test$Fuzz$fuzzLoop : undefined;
var $elm_explorations$test$Test$Fuzz$fuzzLoop = F2(function (c, state) {
