...
const testVariantDefinition = /^var\s+\$elm_explorations\$test\$Test\$Internal\$(UnitTest|FuzzTest|Labeled|Skipped|Only|Batch)\s*=\s*(?:\w+\(\s*)?function\s*\([\w, ]*\)\s*\{\s*return\s*\{\s*\$:\s*(['"])\1\2/gm;
...
function generateMainModule(
  ...
  var opts = {
    ...
    globs: makeList(testFileGlobs.map(sanitizedToString)),
    paths: makeList(testFilePaths.map(sanitizedToString)),
  };

  const testFileBody = makeTestFileBody(testModules, opts);
...
, globs =
    ${indentAllButFirstLine('    ', opts.globs)}
, paths =
    ${indentAllButFirstLine('    ', opts.paths)}
}
  `.trim();
}

function makeTestFileBody(testModules, opts) {
  const imports = testModules.map((mod) => `import ${mod.moduleName}`);

  const maybeTestList = makeList(testModules.map(makeModuleTuple));
...
main =
    Test.Runner.Node.run
        ${indentAllButFirstLine('        ', makeOptsCode(opts))}
        ${indentAllButFirstLine('        ', maybeTestList)}
  `.trim();
}
