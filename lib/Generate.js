...
// Stores all we need to know about a previous run to determine
// which tests haven’t changed and therefore can be skipped.
// TODO: Do we store test results? Or do we somehow only store the ones that passed?
const fingerprintsFileName = 'fingerprints.json';
 ...
 * @returns { void }
 */
function prepareCompiledJsFile(fuzz, seed, testModules, pipeFilename, dest) {
  ...
  fs.writeFileSync(
    path.join(path.dirname(dest), fingerprintsFileName),
 ...
 */
function readOldFingerprints(dest) {
  try {
    return JSON.parse(
      fs.readFileSync(
        path.join(path.dirname(dest), fingerprintsFileName),
...
function addKernelTestChecking(hashes, content) {
  return (
    ...
    content
      ...
      .replace(
        checkDefinition,
        // TODO: Also care about fuzz and seed, and don’t just filter them away.
        '$1 = F3((value, oldHash, newHash) => value && value.__elmTestSymbol === __elmTestSymbol && oldHash !== newHash ? $elm$core$Maybe$Just(value) : $elm$core$Maybe$Nothing);'
      )
...
/**
 * @param { Fingerprints } oldFingerprints
 * @param { number } fuzz
 * @param { number } seed
...
function makeTestFileBody(oldFingerprints, testModules, optsCode) {
  ...
  const possiblyTestsList = makeList(
    testModules.map((mod) => makeModuleTuple(oldFingerprints, mod))
  );
 ...
 * @returns { string }
 */
function makeModuleTuple(oldFingerprints, mod) {
  const list = mod.possiblyTests.map((test) => {
    const name = toCompiledJavaScriptName(mod.moduleName, test);
    // TODO: Also care about fuzz and seed.
    const oldHash = oldFingerprints.hashes[name] || '';
    const newHash = makeHashPlaceholder(name);
    return `Test.Runner.Node.check ${mod.moduleName}.${test} "${oldHash}" "${newHash}"`;
  });
