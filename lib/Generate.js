...
const detectFuzzTestAndDebugLogDefinition =
  /^(var\s+\$author\$project\$Test\$Runner\$Node\$detectFuzzTestAndDebugLog)\s*=\s*\$author\$project\$Test\$Runner\$Node\$detectFuzzTestAndDebugLogHelperReplaceMe___;?$/m;

const fuzzLoopDefinition =
  /^var \$elm_explorations\$test\$Test\$Fuzz\$fuzzLoop\s*=\s*F\d\(\s*function\s*\([^()]*\)\s*\{/m;

const debugLogDefinition =
  /^var _Debug_log\s*=\s*F\d\(\s*function\s*\([^()]*\)\s*\{/m;
...
function patch(hashes, content) {
  return (
    ...
    content
      ...
      .replace(
        checkDefinition,
        // Abuse `$elm_explorations$test$Test$Internal$duplicatedName` to get the test name(s).
        // Usually, a `Test` has a single name (from `test`, `fuzz` or `describe`), but `Test.concat`
        // does not have a name – instead we need to use the names of all the tests it concatenates
        // (recursively). `$elm_explorations$test$Test$Internal$duplicatedName` returns the unique
        // names when there are no duplicates – and there shouldn’t be, because that is not constructable.
        `
$1 = F3((test, jsDefinitionName, hash) =>
  test && test.__elmTestSymbol === __elmTestSymbol
    ? $elm$core$Maybe$Just({ test, jsDefinitionName, hash, labels: $elm_explorations$test$Test$Internal$duplicatedName(_List_Cons(test, _List_Nil)).a })
...
`.trim()
      ...
      .replace(
        detectFuzzTestAndDebugLogDefinition,
        `
var _elmTestIsFuzzTest = false;
var _elmTestUsedDebugLog = false;
$1 = (f) => {
  _elmTestIsFuzzTest = false;
  _elmTestUsedDebugLog = false;
  return _Utils_Tuple3(f(null), _elmTestIsFuzzTest, _elmTestUsedDebugLog);
}
`.trim()
      )
      .replace(fuzzLoopDefinition, '$& _elmTestIsFuzzTest = true;')
      .replace(debugLogDefinition, '$& _elmTestUsedDebugLog = true;')
...
function generatePreviousRunModule(previousRunModule, previousRun) {
  const fingerprintsList = makeList(
    Object.entries(previousRun.fingerprints)
      // If there are no outcomes, the value wasn’t a test.
      // Remember that we collect _all_ exposed values and test them at runtime if they are tests or not.
      .filter(([, { outcomes }]) => outcomes.length > 0)
      .map(([jsIdentifierName, { hash, outcomes }]) =>
        `
...
import Test.Reporter.TestResults exposing (Outcome(..))
