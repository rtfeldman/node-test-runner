...
async function installUnstableTestMaster() {
  ...
  const zipballUrl = `https://github.com/${pkg}/archive/refs/heads/master.zip`;
  const zipballPath = `${tempDir}/${pkgWithDash}.zip`;
  ...
  await ourExec(`rm -rf elm-stuff/generated-code/elm-community/elm-test`);
  await ourExec(`mkdir -p ${packagePath}`);
  await ourExec(`curl -L ${zipballUrl} -o ${zipballPath}`);
  await ourExec(`unzip -o ${zipballPath} -d ${tempDir}`);
  await ourExec(`mv ${tempDir}/test-master/src ${packagePath}`);
  await ourExec(`mv ${tempDir}/test-master/elm.json ${packagePath}`);
  await ourExec(`mv ${tempDir}/test-master/LICENSE ${packagePath}`);
  await ourExec(`mv ${tempDir}/test-master/README.md ${packagePath}`);
