 ...
 * @returns { 'SuccessfullyInstalled' | 'AlreadyInstalled' }
 */
function install(project, pathToElmBinary, packageName) {
  ...
  const result = Spawn.spawnSync(pathToElmBinary, ['install', packageName], {
    stdio: 'inherit',
  ...
  if (newElmJson.type === 'package') {
    ...
    // This variable is needed for TypeScript to remember that `newElmJson` has
    // been narrowed to `application` inside the `moveToTestDeps` function.
    const newElmJson_ = newElmJson;
    /**
     * @param { 'direct' | 'indirect' } directness
     * @returns { void }
     */
    function moveToTestDeps(directness) {
      Object.keys(newElmJson_['dependencies'][directness]).forEach(function (
        key
      ) {
        // If we didn't have this dep before, move it to test-dependencies.
        if (!elmJson['dependencies'][directness].hasOwnProperty(key)) {
          ...
          if (
            directness === 'direct' ||
            !newElmJson_['test-dependencies']['direct'].hasOwnProperty(key)
          ) {
            newElmJson_['test-dependencies'][directness][key] =
              newElmJson_['dependencies'][directness][key];
          }

          delete newElmJson_['dependencies'][directness][key];
        }
      });
    }
