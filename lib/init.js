...
function init(args) {
  if (fs.existsSync("elm-package.json")) {
    try {
      ...
      var newElmPackageContents = fs.stringify(modifyElmPackage(elmPackageContents), null, 4);
      var dest = path.resolve(path.join("test", "elm-package.json"));
  ...
  ensureDirectory(path.join("test", "suites"));
  ensureDirectory(path.join("test", "shared"));
  copyTemplate(path.join("suites", "Tests.elm"));
...
function copyTemplate(templateName, destName) {
  ...
  var destination = path.resolve("test", destName);
