 ...
 * @returns { {
    ...
    newLine: () => void,
    overwrite: (message: string) => void,
    clearLine: () => void,
    clearConsole: () => void,
  } }
 */
function makeProgressLogger(report, clearConsole) {
  /** @type { Array<string> } */
  const items = [];
  return {
    log(message) {
 ...
 * @returns { string }
 */
function watcherEventMessage(queue) {
...
function runTests(
  ...
  testFileGlobs,
  processes,
  { watch, clearConsole, report, seed, fuzz }
) {
  ...
  async function run() {
    try {
      ...
      console.time('ElmJson.requireElmTestPackage');
      ElmJson.requireElmTestPackage(projectRootDir, project.elmJson);
      console.timeEnd('ElmJson.requireElmTestPackage');
      console.time('Project.validateTestsSourceDirs');
      Project.validateTestsSourceDirs(project);
      console.timeEnd('Project.validateTestsSourceDirs');

      console.time('FindTests.resolveGlobs');
      ...
      console.timeEnd('FindTests.resolveGlobs');
      ...
      console.time('FindTests.findTests');
      const testModules = await FindTests.findTests(testFilePaths, project);
      console.timeEnd('FindTests.findTests');
      const mainModule = Generate.getModule(
        project.generatedCodeDir,
        Generate.mainModuleName
      );
      const previousRunModule = Generate.getModule(
        project.generatedCodeDir,
        Generate.previousRunModuleName
      );
      const dest = path.join(project.generatedCodeDir, 'elmTestOutput.js');

      console.time('Generate.generateElmJson');
      Generate.generateElmJson(dependencyProvider, project);
      console.timeEnd('Generate.generateElmJson');

      progressLogger.log('Compiling');

      console.time('Generate.generateMainModule');
      ...
      console.timeEnd('Generate.generateMainModule');
      console.time('Generate.ensurePreviousRunModule');
      Generate.ensurePreviousRunModule(previousRunModule);
      console.timeEnd('Generate.ensurePreviousRunModule');

      console.time('Compile.compile');
      ...
      console.timeEnd('Compile.compile');

      console.time('Generate.prepareCompiledJsFile');
      const hashes = Generate.prepareCompiledJsFile(
        testModules,
        pipeFilename,
        dest
      );
      console.timeEnd('Generate.prepareCompiledJsFile');

      progressLogger.log('Starting tests');
      progressLogger.newLine();

      console.time('Supervisor.run');
      ...
      console.timeEnd('Supervisor.run');
      return r;
    } catch (err) {
      progressLogger.newLine();
  ...
  if (watch) {
    ...
    /** @type { (event: 'added' | 'changed' | 'removed') => (absoluteFilePath: string) => void } */
