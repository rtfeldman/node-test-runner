...
function getDependenciesCached(
  ...
  const dependencies = getDependencies(project.elmJson);
...
function getDependencies(elmJson /*: typeof ElmJson.ElmJson */) /*: string */ {
  const useTest = true;
  const extra = {
  ...
  let solution;
  try {
    const offlineDepsProvider = DependencyProvider.newOffline();
    solution = offlineDepsProvider.solve(
      JSON.stringify(elmJson),
      useTest,
      extra
    );
  } catch (_) {
    // console.error('Offline solver failed, switching to online');
    const onlineDepsProvider = DependencyProvider.newOnline();
    solution = onlineDepsProvider.solve(
      JSON.stringify(elmJson),
      useTest,
      extra
    );
