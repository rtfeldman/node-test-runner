...
let depsProvider = require('./dependency-provider.js');
...
async function getDependencies(
  ...
  let solution;
  try {
    depsProvider.clearListVersionsMemoCacheBeforeSolve();
    solution = wasm.solve_deps(
      elmJsonConfig,
      useTest,
      extra,
      depsProvider.fetchElmJsonOffline,
      depsProvider.listAvailableVersionsOffline
    );
  } catch (_) {
    console.log('Offline solver failed, switching to online');
    // Update the online cache of existing versions.
    depsProvider.updateOnlineVersionsCache();
    ...
    solution = wasm.solve_deps(
      ...
      useTest,
      extra,
      depsProvider.fetchElmJsonOnline,
      depsProvider.listAvailableVersionsOnline
    );
  }
  return solution;
}
