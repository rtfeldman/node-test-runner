...
const Package = require("../cli/package.js");
...
describe("mapping repository to native package name", () => {
  ...
  it("should map repository to native package name", () => {
    const repository = "https://github.com/rtfeldman/node-test-Package.git";
    const expectedNativePackageName = "rtfeldman$node_test_runner";

    assert.equal(
      Package.repositoryToNativePackageName(repository),
  ...
  it("should throw error if user name and repository name cannot be parsed from repository", () => {
    ...
    const expectedErrorMessage =
      "Unable to convert repository malformedRepository to package name.";

    assertThrowsErrorMessage(
      () => Package.repositoryToNativePackageName(malformedRepository),
  ...
  it("should throw error if repository name is empty", () => {
    ...
    assertThrowsErrorMessage(
      () => Package.repositoryToNativePackageName(malformedRepositoryName),
      expectedErrorMessage
    );
  });

  it("should throw error if user name is empty", () => {
    const malformedRepositoryName = "https://github.com//node-test-Package.git";
    const expectedErrorMessage =
      "Unable to convert repository " +
      malformedRepositoryName +
      " to package name.";
  ...
  it("should throw error if user name includes dots", () => {
    const malformedRepositoryName =
      "https://github.com/user.with.dots/node-test-Package.git";
    const expectedErrorMessage =
  ...
  it("should throw error if repository name includes dots", () => {
    const malformedRepositoryName =
      "https://github.com/antivanov/underscore.elm.git";
    const expectedErrorMessage =
      "Elm currently doesn't support having periods in the user/project part of the repository field of elm-package.json. Aborting test run.";
